...
class Melolo : MainAPI() {
        ...
        val responseText = executeWithRetry {
            ...
            app
                .post(
                    ...
                    headers = mapOf(
                        "Content-Type" to "application/json", "X-Xs-From-Web" to "false", "User-Agent" to "okhttp/4.9.3",
                        "Referer" to "$mainUrl/"
                    )
                ).text
        }
    ...
    private suspend fun fetchDetail(bookId: String): CatalogVideoData {
        ...
        return tryParseJson<CatalogDetailResponse>(res.text)?.data?.video_data
            ?: throw ErrorLoadingException("Empty detail data")
    }
