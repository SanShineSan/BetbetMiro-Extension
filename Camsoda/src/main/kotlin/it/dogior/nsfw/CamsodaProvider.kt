...
class CamsodaProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val responseList = app.get("$mainUrl${request.data}&p=$page")
            .parsedSafe<Response>()!!.userList.mapNotNull { user ->
                if (user.status!="private") {
                    newLiveSearchResponse(
                        ...
                        this.posterUrl = user.thumbUrl.ifEmpty { user.offlinePictureUrl }
                        this.lang = null
                    }
                }else{
                    null
        ...
        for (i in 0..3) {
            val results = app.get("$mainUrl/api/v1/browse/react/search/$query?p=$i&perPage=98")
                .parsedSafe<Response>()!!.userList.map { user ->
                    newLiveSearchResponse(
                        name = user.username + " - " + user.status.replace("_ai", ""),
                        url = "$mainUrl/${user.username}",
                        type = TvType.Live
                    ) {
                        this.posterUrl = user.offlinePictureUrl.ifEmpty { user.thumbUrl }
                        this.lang = null
                    }
                    /*LiveSearchResponse(
                        name = user.username,
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val isPrivate = document.toString().contains("\"status\":\"private\"")
        if (isPrivate) title = "$title - private"
